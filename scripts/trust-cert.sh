...
# Use arrays for safe iteration
mapfile -t CERTS < <(find "$SSL_DIR" -name "*.crt" -not -name "*.ca.crt")
mapfile -t CA_CERTS < <(find "$SSL_DIR" -name "*.ca.crt")

# List available certificates
if [[ ${#CERTS[ @ ]} -eq 0 ]] && [[ ${#CA_CERTS[ @ ]} -eq 0 ]]; then
...
case "$OS" in
  Darwin*)
    ...
    printf '6. Close the window (you will need to enter your password)\n'
  ;;
...
printf '%bIf you are still experiencing certificate warnings:%b\n' "$YELLOW" "$NC"
printf '1. For Chrome/Edge, you can type %bthisisunsafe%b directly on the certificate warning page\n' "$GREEN" "$NC"
printf '   (you will not see what you are typing, but it will bypass the warning)\n'
...
printf '2. Clear your browser cache if you are still having issues\n'
