...
─ registry, daemon lifecycle      (the cluster-sni.json pattern)  poll/io*uring loop
```
...
N copies with `SO*REUSEPORT` for multi-core (the bench spawns them, exactly like
...
zig build-exe src/main.zig -O ReleaseFast -target x86*64-linux-musl -femit-bin=/tmp/rpx-dp
```
...
2. **io*uring** event loop (Linux 5.x): multishot accept, batched SQEs, registered
